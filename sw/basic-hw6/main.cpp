...
// Filter ACC
static char* const GAUSSIANBLURFILTER_START_ADDR = reinterpret_cast<char* const>(0x73000000);
static char* const GAUSSIANBLURFILTER_READ_ADDR  = reinterpret_cast<char* const>(0x73000004);
    ...
    for(int j = 0; j < length; j++){
      for(int v = -1; v <= 1; v ++){
        for(int u = -1; u <= 1; u++){
          ...
          write_data_to_ACC(GAUSSIANBLURFILTER_START_ADDR, buffer, 4);
        }
      }
      read_data_from_ACC(GAUSSIANBLURFILTER_READ_ADDR, buffer, 4);
